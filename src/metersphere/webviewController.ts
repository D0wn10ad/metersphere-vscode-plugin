    ...
    this.panel.webview.onDidReceiveMessage(async (message) => {
      ...
      if (command === 'sendRequest') {
        try {
          ...
          // Save to history (with original URL for display)
          this.saveToHistory({ url: reqPayload.url, method: reqPayload.method }, resp)
        ...
        if (envId) {
          const env = this.context.workspaceState.get<any>('cachedEnvironments')?.find((e: any) => e.id === envId)
          if (env) {
